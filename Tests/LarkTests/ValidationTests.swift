...
class FaultTests: XCTestCase {
    func testDeserialization() throws {
        let element = try XMLElement(xmlString: "<fault xmlns:soap=\"\(NS_SOAP)\"><faultcode>soap:Server</faultcode><faultstring>NullReferenceException</faultstring><faultactor>http://tempuri.org/my-server</faultactor><detail><foo>bar</foo></detail></fault>")
        let fault = try Fault(deserialize: element)
        XCTAssertEqual(fault.faultcode, QualifiedName(uri: NS_SOAP, localName: "Server"))
    ...
    func testDescription() {
        let fault = Fault(faultcode: QualifiedName(uri: NS_SOAP, localName: "VersionMismatch"), faultstring: "version>=4", faultactor: nil, detail: [])
